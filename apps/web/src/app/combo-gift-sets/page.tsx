...
import ShopAllContainer from '../shop-all/ShopAllContainer';
...
const CombosAndGiftSets = async (): Promise<JSX.Element> => {
  ...
  try {
    ...
    logError(JSON.stringify(err), 'CombosAndGiftSets');
  ...
  return (
    <Suspense fallback={<ShopAllSkeletonView />}>
      <div className="overflow-hidden max-h-[300px]">
        <BannerContainer filter={BannerType.combosAndGiftSets} />
      </div>
      <ShopAllContainer
        heading="Combos and Gift sets"
        filterData={{ ...filterData, common: commonFilter }}
        productData={productData?.filter((a) => a.isComboOrGift)}
      />
    </Suspense>
  );
};

export default CombosAndGiftSets;
