...
import { deleteProductInterest } from './productInterest.action';
import PermissionCheck from '../../../lib/PermissionCheck';
import { permissions } from '@repo/ui/lib';

export interface IDeleteProductInterestContainerProps {
  id: number;
  title: string;
  onClose: () => void;
  onDelete: () => void;
...
export default function DeleteProductInterestContainer({
  id,
  title,
  onClose,
  onDelete,
}: IDeleteProductInterestContainerProps) {
  const [value, setValue] = useState<string>('');
  const [loading, setLoading] = useState<boolean>(false);

  const handleSubmit = async () => {
    setLoading(true);
    const data = await deleteProductInterest(id);
    if ('error' in data) {
