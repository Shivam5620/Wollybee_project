...
import { deleteProductAge } from './productAge.action';
import { permissions } from '@repo/ui/lib';
import PermissionCheck from '../../../lib/PermissionCheck';

export interface IDeleteProductAgeContainerProps {
  id: number;
  title: string;
  onClose: () => void;
  onDelete: () => void;
...
export default function DeleteProductAgeContainer({
  id,
  title,
  onClose,
  onDelete,
}: IDeleteProductAgeContainerProps) {
  const [value, setValue] = useState<string>('');
  const [loading, setLoading] = useState<boolean>(false);

  const handleSubmit = async () => {
    setLoading(true);
    const data = await deleteProductAge(id);
    if ('error' in data) {
